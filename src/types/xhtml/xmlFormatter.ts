class XMLFormatter {
    ...
    static deprettifyFromRoot = (deepcopy: boolean) => (node: Node) => {
    ...
    static inXMLSpacePreserve(node: Node) {
    ...
    static notInXMLSpacePreserve(node: Node) {
        return !XMLFormatter.inXMLSpacePreserve(node)
    }

    static prettifyFromRoot(deepcopy: boolean) {
    
        function _prettifyFromRoot(node1: Node) {

            function _prettifyDescendants(node2: Node) {

                const depth = XML.depth(node2)
                if (depth === 0) return node2
                if (XMLFormatter.inXMLSpacePreserve(node2)) return node2
